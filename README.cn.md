...
| DOCKER_STOP_GRACE_PERIOD  | 否  | 容器终止（docker stop）时的优雅等待时间（秒）, 默认为 `10`。（示例：`30`）                                          |
| DOCKER_RUN_PARAMS         | 否  | 启动容器时传递的额外运行参数，可以包括环境变量、端口映射、文件夹映射等。（示例：`-e ENV=prod -e TZ=Asia/Shanghai`）           |
| HEALTHCHECK_URL           | 否  | 健康检查（Readiness）URL，用于判断新启动的容器是否已经完成初始化并具备对外提供服务的能力。（示例：`http://127.0.0.1/ping`）   |
