...
| 变量名                    | 必须 | 描述                                                                                      |
|------------------------|----|-----------------------------------------------------------------------------------------|
| DOCKER_IMAGE           | 是  | Docker 镜像地址，用于拉取并启动指定的应用容器。（如：`example_ns/myapp`）                                       |
| CONTAINER_NAME         | 是  | Docker 容器名称，确保不与其他容器冲突。（如：`my_container`）                                               |
| DOCKER_IMAGE_TAG       | 否  | 镜像和容器部署的版本标签，用于指定 Docker 镜像的版本。（如：`latest`），默认为 `latest`。                               |
| DOCKER_RUN_PARAMS      | 否  | 启动容器时传递的额外运行参数（如：`-e ENV=prod`），可以包括环境变量、端口映射、文件夹映射等。                                   |
| DOCKER_REGISTRY_URL    | 否  | Docker 私有仓库的 URL 地址，若为空，则默认使用官方的 DockerHub。（如：`https://index.docker.io/v1`）             |
...
| SERVER_HOST            | 否  | 服务器的主机名或 IP 地址，用于通过 SSH 连接到目标服务器。在 `AUTH_METHOD` 为 `pwd` 或 `key` 时必填。（如：`192.168.1.100`） |
| SERVER_USER            | 否  | 服务器登录用户名，清确保该用户具备操作 Docker 的权限。在 `AUTH_METHOD` 为 `pwd` 或 `key` 时必填。（如：`root`）           |
| SERVER_PASSWORD        | 否  | 服务器登录密码，在 `AUTH_METHOD` 为 `pwd` 时必填。 （如：`mypassword`）                                   |
| SERVER_SSH_PRIVATE_KEY | 否  | 服务器SSH 私钥内容，用于密钥登录服务器。在 `AUTH_METHOD` 为 `key` 时必填。（如：`-----BEGIN PRIVATE KEY-----`）     |
